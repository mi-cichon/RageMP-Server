...
window.onload = function(){
    ...
    for(let i=0; i<itemsCount; i++){
        ...
        body.append(`
        <div class="roller_item" style="background-color: ${rarities[reward.rarity-1]}">
        <div class="roller_img" style="background-image: url(img/${reward.img})"></div>
        </div>
        `);
        if(i==79){
            rewardId = reward.id;
...
function scroll(){
    let current = 0;
    let body = $(".roller_scrollbody");
    let scrollWidth = body.width();
    let bodyWidth = $(".roller_body").width();
    let itemWidth = $(".roller_item").width();
...
function scrollInt(current, toScroll, step){
    ...
    scrollInterval = setInterval(()=>{
        ...
        if(current >= toScroll){
            setTimeout(()=>{
                mp.trigger("bonus_reward", rewardId);
            }, 2000);
            clearInterval(scrollInterval);
        }
