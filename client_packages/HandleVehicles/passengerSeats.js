...
mp.keys.bind(71, false, () => {
  if (mp.players.local.vehicle === null && !mp.gui.cursor.visible && !mp.players.local.getVariable("controlsblocked")  && (mp.players.local.getVariable("job") === "" || mp.players.local.getVariable("job") == "lspd")) {
    ...
    if (vehicle.isAVehicle()) {
      if (vehicle.getVariable('veh_locked') === true) {
        mp.events.call("showNotification", "Pojazd jest zamknięty!");
        return; // No need to calc stuff, vehicle is locked.
      }
      if(!(vehicle.getVariable("type") != "dealer" && vehicle.getVariable("type") != "public" && !vehicle.isSeatFree(-1) && !(vehicle.hasVariable("licence")))){
          return;
      }
