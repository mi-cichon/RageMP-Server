function insertData(name, price, trip){

}

function setVehiclesData(seller, price, vehInfo, mechInfo, visuInfo){
    vehInfo = JSON.parse(vehInfo);
    $('.seller').text("Od: " + seller);
    let name = vehInfo[0];
    let id = vehInfo[1];
    let state = vehInfo[2];
    let comb = vehInfo[3];
    let tank = vehInfo[4];
    let trunk = vehInfo[5];
    let trip = vehInfo[6];
    let mechtune = mechInfo == "" ? "brak" : JSON.parse(mechInfo).join(', ');
    let visutune = visuInfo == "" ? "brak" : JSON.parse(visuInfo).join(', ');

    $(".name").text(name);
    $('.trip').text(parseFloat(trip).toFixed(1) + " km");
    $(".price").text(betterNumbers(price) + "$");
    $(".mechtune").text(mechtune);
    $(".visutune").text(visutune);
}

$(".button_cancel").on("click", function(){
    mp.trigger("carTrade_closeBrowser");
});

$(".button_select").on("click", function(){
    mp.trigger("carTrade_confirmTrade");
    mp.trigger("carTrade_closeBrowser");
});

function betterNumbers(val) {
    let spacer = ' ';
    let result = val.toString().trim();
    for(var i = val.toString().trim().length-1; i >= 0; i--) {
        if((val.toString().trim().length - i) % 3 == 0) {
            if(val.toString().slice(0,i) != '')
            result = result.slice(0,i) + spacer + result.slice(i);
        }
    }
    return result;
}